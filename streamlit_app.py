...
def get_balance_status(x):
    """Get balance status with safe comparison"""
    try:
        val = safe_float(x)
        if val < 0:
            return "🟢 They Owe You"
...
def clean_dataframe(df):
    """Clean and ensure proper data types in dataframe"""
    if df.empty:
        return df
    ...
    numeric_cols = ['Quantity', 'Price per Unit', 'Total Amount', 'Balance Effect']
    for col in numeric_cols:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
    
    # Ensure string columns
    string_cols = ['Party Name', 'Type', 'Item Name', 'Description']
    ...
    # Ensure Balance Effect column exists and is calculated
        ...
        for _, row in recent.iterrows():
            ...
            st.markdown(f"""
                <div style="background-color: #f5f5f5; padding: 10px 15px; border-radius: 8px; 
                    ...
                    <span style="color: #bf360c;">Qty: {int(abs(safe_float(row['Quantity'])))} @ ₹{safe_float(row['Price per Unit']):,.2f}</span>
                </div>
            """, unsafe_allow_html=True)
    ...
    if not view_df.empty:
        ...
        for _, row in page_df.iterrows():
            ...
            with st.container():
                ...
                with col1:
                    qty_display = int(abs(safe_float(row['Quantity'])))
                    amount_display = abs(safe_float(row['Total Amount']))
                    price_display = safe_float(row['Price per Unit'])
                    st.markdown(f"""
                        <div style="background-color: #fafafa; padding: 10px 15px; border-radius: 8px; 
                            ...
                            <span style="color: #6a1b9a;"><strong>{row['Type']}</strong></span> | 
                            <span style="color: #00695c;">{row['Item Name']}</span> | 
                            <span style="color: #bf360c;">Qty: {qty_display} @ ₹{price_display:,.2f}</span> → 
                ...
                with st.form("edit_form"):
                    ...
                    with edit_col2:
                        ...
                        new_qty_raw = st.number_input(
                            "Quantity (absolute)", 
                            value=int(abs(safe_float(row['Quantity']))), 
                            min_value=1
                        )
                        new_price = st.number_input(
                            "Price per Unit", 
                            value=float(safe_float(row['Price per Unit'])), 
                            min_value=0.0,
                            step=0.5
                        )
                    ...
                    if save_btn:
                        if not sanitize_input(new_party) or not sanitize_input(new_item):
                            ...
                            # Update using proper column assignment
                            ...
                            st.session_state.df.at[edit_idx, 'Quantity'] = float(new_qty)
                            st.session_state.df.at[edit_idx, 'Price per Unit'] = float(new_price)
                            st.session_state.df.at[edit_idx, 'Total Amount'] = float(new_total)
                            st.session_state.df.at[edit_idx, 'Balance Effect'] = float(new_balance_effect)
                            st.session_state.df.at[edit_idx, 'Description'] = sanitize_input(new_description)
        ...
        st.markdown(f"""
            <div style="background-color: #e3f2fd; padding: 15px; border-radius: 8px; 
                ...
                <span style="color: #424242;">
                    {trans_type} | Stock Effect: <strong>{'+' if preview_qty > 0 else ''}{int(preview_qty)}</strong> | 
        ...
        if submitted:
            ...
            if not party:
                ...
                new_row = pd.DataFrame([{
                    ...
                    'Balance Effect': float(balance_effect),
    ...
    with format_col1:
        st.markdown("""
            <div style="background-color: #e8f5e9; padding: 15px; border-radius: 8px; margin: 10px 0;">
                <strong style="color: #2e7d32;">📦 Items CSV:</strong>
                <pre style="background-color: #c8e6c9; padding: 10px; border-radius: 5px; margin-top: 10px; color: #1b5e20;">Item Name
Rice 25kg
Sugar 10kg</pre>
            </div>
        """, unsafe_allow_html=True)
        
        sample_items = pd.DataFrame({"Item Name": ["Rice 25kg", "Sugar 10kg", "Wheat Flour 5kg"]})
        st.download_button("📥 Sample Items CSV", sample_items.to_csv(index=False).encode('utf-8'), "sample_items.csv", "text/csv", use_container_width=True)
    
    with format_col2:
        st.markdown("""
            <div style="background-color: #e3f2fd; padding: 15px; border-radius: 8px; margin: 10px 0;">
                <strong style="color: #1565c0;">👥 Parties CSV:</strong>
                <pre style="background-color: #bbdefb; padding: 10px; border-radius: 5px; margin-top: 10px; color: #0d47a1;">Party Name
Sharma Traders
ABC Distributors</pre>
            </div>
        """, unsafe_allow_html=True)
        
        sample_parties = pd.DataFrame({"Party Name": ["Sharma Traders", "ABC Distributors", "XYZ Wholesale"]})
        st.download_button("📥 Sample Parties CSV", sample_parties.to_csv(index=False).encode('utf-8'), "sample_parties.csv", "text/csv", use_container_width=True)
...
with tab5:
    ...
    if df.empty:
        ...
        if filtered_df.empty:
            ...
            # Ensure Balance Effect is numeric
            ...
            if selected_party_detail != "-- Select --":
                ...
                party_purchases = safe_float(party_transactions[party_transactions['Type'].isin(['Purchase', 'Return In'])]['Total Amount'].sum())
                party_sales = safe_float(party_transactions[party_transactions['Type'].isin(['Sale', 'Return Out'])]['Total Amount'].sum())
                party_receipts = safe_float(party_transactions[party_transactions['Type'] == 'Receipt']['Total Amount'].sum())
                party_payments = safe_float(party_transactions[party_transactions['Type'] == 'Payment']['Total Amount'].sum())
...
with tab6:
    ...
    if df.empty:
        ...
        if report_type == "Stock Summary":
            ...
            display_stock['Stock Value'] = display_stock['Stock Value'].apply(lambda x: f"₹{safe_float(x):,.2f}")
            display_stock['Current Stock'] = display_stock['Current Stock'].apply(lambda x: int(safe_float(x)))
                ...
                for _, row in type_summary.iterrows():
                    ...
                    st.markdown(f"""
                        <div style="background-color: #f5f5f5; padding: 10px 15px; border-radius: 8px; 
                            ...
                            {int(safe_float(row['Total Quantity']))} units | ₹{safe_float(row['Total Amount']):,.2f}</span>
                        </div>
                    """, unsafe_allow_html=True)
            ...
            if not valid_dates.empty:
                ...
                for col in display_monthly.columns:
                    display_monthly[col] = display_monthly[col].apply(lambda x: f"₹{safe_float(x):,.2f}")
                st.dataframe(display_monthly, use_container_width=True)
                
                st.markdown("**Trend Chart**")
                st.line_chart(monthly_pivot)
            else:
                st.warning("No dated transactions available.")
            ...
            if selected_item == "-- All Items --":
                ...
                display_item_summary['Net Stock Change'] = display_item_summary['Net Stock Change'].apply(lambda x: int(safe_float(x)))
                display_item_summary['Total Value'] = display_item_summary['Total Value'].apply(lambda x: f"₹{safe_float(x):,.2f}")
                ...
                if not item_df.empty:
                    ...
                    with m_col4:
                        display_metric_card("💰 Avg Price", f"₹{safe_float(avg_price):,.0f}" if pd.notna(avg_price) else "N/A", "purple")
                    ...
                    display_item_df['Quantity'] = display_item_df['Quantity'].apply(lambda x: int(safe_float(x)))
                    display_item_df['Price per Unit'] = display_item_df['Price per Unit'].apply(lambda x: f"₹{safe_float(x):,.2f}")
                    display_item_df['Total Amount'] = display_item_df['Total Amount'].apply(lambda x: f"₹{safe_float(x):,.2f}")
            ...
            if selected_party == "-- All Parties --":
                ...
                display_party_summary['Total Value'] = display_party_summary['Total Value'].apply(lambda x: f"₹{safe_float(x):,.2f}")
                ...
                if not party_df.empty:
                    ...
                    total_value = safe_float(party_df['Total Amount'].sum())
                    ...
                    display_party_df['Quantity'] = display_party_df['Quantity'].apply(lambda x: int(safe_float(x)))
                    display_party_df['Price per Unit'] = display_party_df['Price per Unit'].apply(lambda x: f"₹{safe_float(x):,.2f}")
                    display_party_df['Total Amount'] = display_party_df['Total Amount'].apply(lambda x: f"₹{safe_float(x):,.2f}")
